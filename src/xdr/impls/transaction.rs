...
use crate::{
    types::{
        FeeBumpTransactionEnvelope, FeeBumpTransactionExt, FeeBumpTransactionInnerTx, Memo, MuxedAccount, Operation,
        Preconditions, TimeBounds, Transaction, TransactionEnvelope, TransactionExt, TransactionV0, TransactionV0Ext,
...
impl Transaction {
    ...
    pub fn into_transaction_envelope(mut self) -> TransactionEnvelope {
        self.fee = self.fee.checked_mul(self.operations.len() as u32).unwrap_or(self.fee);
...
impl From<TransactionV0> for Transaction {
    fn from(transaction: TransactionV0) -> Self {
        let time_bounds = transaction.time_bounds.unwrap_or(TimeBounds::from_time_points((), ()));
    ...
    pub fn new<T: IntoMuxedAccountId, S: IntoAmount>(
        ...
        let v1_envelope = match inner_transaction_envelope {
            TransactionEnvelope::EnvelopeTypeTxV0(envelope) =>
                TransactionV1Envelope { tx: envelope.tx.into(), signatures: envelope.signatures },
            TransactionEnvelope::EnvelopeTypeTx(envelope) => envelope,
            TransactionEnvelope::EnvelopeTypeTxFeeBump(_) => return Err(StellarSdkError::CantWrapFeeBumpTransaction),
...
mod test {
    use crate::{
        network::TEST_NETWORK, types::Preconditions, Asset, IntoSecretKey, Memo, MilliSecondEpochTime, Operation,
        Price, SecondEpochTime, StroopAmount, TimeBounds, Transaction, TransactionEnvelope, XdrCodec,
    };
    ...
    fn build_transaction1() {
        let mut transaction = Transaction::new(
            ...
            1980190376853505,
            Some(100),
            Preconditions::PrecondTime(TimeBounds::from_time_points(SecondEpochTime(0), SecondEpochTime(1626258131))),
            None,
        )
        .unwrap();

        transaction
            .append_operation(Operation::new_payment(ACCOUNT_ID2, Asset::native(), "123.456").unwrap())
            .unwrap();
        ...
        assert_eq!(envelope, TransactionEnvelope::from_base64_xdr(expexted_base64).unwrap());
    ...
    fn build_transaction2() {
        let mut transaction = Transaction::new(
            ...
            1980190376853505,
            Some(321),
            Preconditions::PrecondTime(TimeBounds::from_time_points(SecondEpochTime(0), SecondEpochTime(0))),
    ...
    fn build_transaction3() {
        ...
        envelope
            .sign(&TEST_NETWORK, vec![&SIGNER1.into_secret_key().unwrap(), &SIGNER3.into_secret_key().unwrap()])
            .unwrap();
        ...
        assert_eq!(envelope, TransactionEnvelope::from_base64_xdr(expexted_singed_base64).unwrap());
