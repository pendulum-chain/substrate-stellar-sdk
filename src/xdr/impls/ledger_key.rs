use crate::{
    ...
    Asset, Error, IntoAccountId, IntoClaimbleBalanceId, IntoHash, LedgerKey,
};

impl LedgerKey {
    pub fn from_account_id<T: IntoAccountId>(account_id: T) -> Result<Self, Error> {
        let account_id = account_id.into_account_id()?;
        Ok(Self::Account(LedgerKeyAccount { account_id }))
    }

    pub fn from_trustline<T: IntoAccountId>(account_id: T, asset: Asset) -> Result<Self, Error> {
        let account_id = account_id.into_account_id()?;
        Ok(Self::Trustline(LedgerKeyTrustLine { account_id, asset }))
    }

    pub fn from_offer<T: IntoAccountId>(seller_id: T, offer_id: i64) -> Result<Self, Error> {
    ...
    pub fn from_liquidity_pool_id<T: IntoHash>(liquidity_pool_id: T) -> Result<Self, Error> {
