use crate::{
    ...
    types::{
        LedgerKeyAccount, LedgerKeyClaimableBalance, LedgerKeyData, LedgerKeyLiquidityPool, LedgerKeyOffer,
...
impl LedgerKey {
    ...
    pub fn from_trustline<T: IntoAccountId>(account_id: T, asset: TrustLineAsset) -> Result<Self, StellarSdkError> {
        let account_id = account_id.into_account_id()?;
    ...
    pub fn from_offer<T: IntoAccountId>(seller_id: T, offer_id: i64) -> Result<Self, StellarSdkError> {
        ...
        Ok(Self::Offer(LedgerKeyOffer { seller_id, offer_id }))
    }

    pub fn from_data<T: IntoAccountId, S: AsRef<[u8]>>(account_id: T, data_name: S) -> Result<Self, StellarSdkError> {
        let account_id = account_id.into_account_id()?;
        let data_name = LimitedString::new(data_name.as_ref().to_vec())?;
        Ok(Self::Data(LedgerKeyData { account_id, data_name }))
    }

    pub fn from_claimable_balance_id<T: IntoClaimbleBalanceId>(balance_id: T) -> Result<Self, StellarSdkError> {
        let balance_id = balance_id.into_claimable_balance_id()?;
        Ok(Self::ClaimableBalance(LedgerKeyClaimableBalance { balance_id }))
    }
    ...
    pub fn from_liquidity_pool_id<T: IntoHash>(liquidity_pool_id: T) -> Result<Self, StellarSdkError> {
        ...
        Ok(Self::LiquidityPool(LedgerKeyLiquidityPool { liquidity_pool_id }))
    }
}
